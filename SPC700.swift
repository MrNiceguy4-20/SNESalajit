...
final class SPC700 {
    ...
    @inline(__always) func fetch8(_ apu: APU) -> u8 { let v = apu.read8(pc); pc &+= 1; return v }
    ...
    func adc(_ v: u8) {
        ...
        let ov = (~(Int(a) ^ Int(v)) & (Int(a) ^ Int(res)) & 0x80) != 0
        setFlag(SPC700.V, ov)
        a = res
        updateNZ(a)
    }

    @inline(__always) func sbc(_ v: u8) { adc(~v) }

    @inline(__always)
    func cmp(_ lhs: u8, _ rhs: u8) {
