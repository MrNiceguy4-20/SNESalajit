...
enum ROMLoader {
    ...
    static func load(url: URL) throws -> Cartridge {
        ...
        Log.info(String(format: "Internal header offset: 0x%04X, SRAM size: %d bytes", headerOffset, sramSizeBytes))
    ...
    private static func detectMapping(rom: [u8]) -> (Cartridge.Mapping, Int, Int) {
        let lo = scoreHeader(rom: rom, headerOffset: 0x7FC0, expectedMapping: .loROM)
        let hi = scoreHeader(rom: rom, headerOffset: 0xFFC0, expectedMapping: .hiROM)

        let mapping: Cartridge.Mapping = (lo >= hi) ? .loROM : .hiROM
        return (mapping, lo, hi)
    }

    private static func scoreHeader(
        rom: [u8],
        headerOffset: Int,
        expectedMapping: Cartridge.Mapping
    ) -> Int {
        ...
        let mapMode = rom[headerOffset + 0x15] & 0x0F
        let mapScore: Int
        switch (expectedMapping, mapMode) {
        case (.loROM, 0x0): mapScore = 12
        case (.hiROM, 0x1): mapScore = 12
        case (.loROM, 0x1), (.hiROM, 0x0): mapScore = -8
        default: mapScore = 0
        }
