...
enum CPUInstructionTables {
    ...
    static func execute(opcode: u8, cpu: CPU65816, bus: Bus) -> Int {
        ...
        case 0x42: _ = cpu.fetch8(); return 2 // WDM (2-byte NOP)
        case 0x78: cpu.setFlag(.irqDis, true); return 2 // SEI
        case 0x58: cpu.setFlag(.irqDis, false); return 2 // CLI
        case 0x18: cpu.setFlag(.carry, false); return 2 // CLC
        case 0x38: cpu.setFlag(.carry, true); return 2 // SEC
        case 0xD8: cpu.setFlag(.decimal, false); return 2 // CLD
        case 0xF8: cpu.setFlag(.decimal, true); return 2 // SED
        case 0xC2: cpu.rep(cpu.fetch8()); return 3 // REP #imm
        case 0xE2: cpu.sep(cpu.fetch8()); return 3 // SEP #imm
        case 0xB8: cpu.setFlag(.overflow, false); return 2 // CLV
        case 0xFB: cpu.xce(); return 2 // XCE
        case 0xCB: cpu.wai(); return 3 // WAI

        // MARK: - Interrupts
        case 0x00: cpu.serviceInterrupt(.brk); return 7 // BRK
        case 0x02: cpu.serviceInterrupt(.cop); return 7 // COP
        case 0x40: // RTI
            let p = cpu.pull8()
            cpu.setPFromPull(p)
            let pcl = cpu.pull8()
        ...
        case 0x7A: // PLY
            ...
            return 4
        case 0x08: cpu.push8(cpu.pForPush(brk: false)); return 3 // PHP
        case 0x28: cpu.setPFromPull(cpu.pull8()); return 4 // PLP
        case 0x0B: cpu.push16(cpu.r.dp); return 4 // PHD
        case 0x8B: cpu.push8(cpu.r.db); return 3 // PHB
        case 0x4B: cpu.push8(cpu.r.pb); return 3 // PHK
        case 0xAB: // PLB
            let v = cpu.pull8()
            cpu.setDB(v)
            cpu.updateNZ8(v)
            return 4
        case 0xF4: cpu.push16(cpu.fetch16()); return 5 // PEA #imm16
        ...
        case 0x8A: // TXA
            ...
            return 2
        case 0x5B: cpu.setDP(cpu.r.a); cpu.updateNZ16(cpu.r.dp); return 2 // TCD
        case 0x1B: // TCS
            let val = cpu.r.a
        ...
        // MARK: - Increment / Decrement / Shift (Accumulator)
        case 0xE8: // INX
        ...
        case 0x11: return ora_mem(cpu: cpu, bank: cpu.r.db, addr: CPUAddressing.dpIndirectY(cpu: cpu, bus: bus), cycles: 6) // ORA (dp),Y
        ...
        // MARK: - Memory Operations (Modify / Store)
        ...
        // MARK: - Flow Control
        case 0x4C: cpu.setPC(CPUAddressing.abs16(cpu: cpu, bus: bus)); return 3 // JMP abs
        case 0x6C: cpu.setPC(CPUAddressing.absIndirect(cpu: cpu, bus: bus)); return 5 // JMP (abs)
        case 0x20: // JSR abs
        ...
        case 0x60: cpu.setPC(cpu.pull16() &+ 1); return 6 // RTS
        case 0x80: // BRA rel8
            cpu.setPC(cpu.r.pc &+ u16(bitPattern: Int16(CPUAddressing.rel8(cpu: cpu, bus: bus)))); return 3
        case 0xF0: return branch(cpu: cpu, bus: bus, cond: cpu.flag(.zero)) // BEQ
        case 0xD0: return branch(cpu: cpu, bus: bus, cond: !cpu.flag(.zero)) // BNE
        case 0x10: return branch(cpu: cpu, bus: bus, cond: !cpu.flag(.negative)) // BPL
        case 0x30: return branch(cpu: cpu, bus: bus, cond: cpu.flag(.negative)) // BMI
        case 0x90: return branch(cpu: cpu, bus: bus, cond: !cpu.flag(.carry)) // BCC
        case 0xB0: return branch(cpu: cpu, bus: bus, cond: cpu.flag(.carry)) // BCS
        case 0x50: return branch(cpu: cpu, bus: bus, cond: !cpu.flag(.overflow)) // BVC
        case 0x70: return branch(cpu: cpu, bus: bus, cond: cpu.flag(.overflow)) // BVS
        case 0x5C: // JML long
    ...
    // MARK: - Private Helpers (Memory Modify)
    ...
    // MARK: - Private Helpers (ALU Logic)
    ...
    // MARK: - Private Helpers (Arithmetic)
