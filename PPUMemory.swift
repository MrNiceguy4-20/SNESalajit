...
final class PPUMemory {
    ...
    @inline(__always) func readVRAM16(wordAddress: u16) -> u16 {
    ...
    @inline(__always) func writeVRAMLow(wordAddress: u16, value: u8) {
        let byteIndex = (Int(wordAddress) * 2) & 0xFFFF
        vram[byteIndex] = value
    }

    @inline(__always) func writeVRAMHigh(wordAddress: u16, value: u8) {
        let byteIndex = ((Int(wordAddress) * 2) + 1) & 0xFFFF
        vram[byteIndex] = value
    }

    @inline(__always) func readCGRAM16(colorIndex: Int) -> u16 {
    ...
    @inline(__always) func writeCGRAM16(colorIndex: Int, value: u16) {
        let idx = (colorIndex & 0xFF) * 2
        cgram[idx] = u8(value & 0xFF)
        cgram[idx + 1] = u8((value >> 8) & 0x7F)
    }

    @inline(__always) func readOAM(_ addr: Int) -> u8 {
        if addr < 0 || addr >= 544 { return 0 }
        return oam[addr]
    }

    @inline(__always) func writeOAM(_ addr: Int, _ value: u8) {
